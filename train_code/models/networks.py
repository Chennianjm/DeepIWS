...
class UnetSkipConnectionBlock(nn.Module):
    def __init__(self, outer_nc, inner_nc,
        ...
        downconv = nn.Conv2d(outer_nc, inner_nc, kernel_size=4,
                             stride=2, padding=1, bias=use_bias)  # size->size/2
        in_norm = norm_layer(inner_nc)
        prelu = nn.PReLU()
        out_norm = norm_layer(outer_nc)
        Upsample = nn.Upsample(scale_factor=2, mode='nearest')  # size->2*size
        conv_upsample = nn.Conv2d(inner_nc * 2, outer_nc, kernel_size=3, stride=1, padding=1)

        if outermost:
            conv1 = nn.Conv2d(2, inner_nc, kernel_size=3, stride=1, padding=1)
